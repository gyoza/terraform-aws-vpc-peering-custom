locals {
  ...
  acceptor_vpc_info     = format("%#v", data.aws_route_tables.peer_vpc_rts)
  requestor_vpc_info    = format("%#v", data.aws_route_tables.this_vpc_rts)
}

output acceptor_vpc_info {
  value = local.acceptor_vpc_info
}

output requestor_vpc_info {
  value = local.requestor_vpc_info
}
